...
import { Match, MatchHistoryEntry, NewMatchRequest, MatchUpdateRequest } from "../schemas/matchSchema.js";
...
export async function matchRoutes(fastify: FastifyInstance) {
  ...
  fastify.post<{ Body: NewMatchRequest }>("/", async (req, reply) => {
    try {
      ...
      if (!["1vs1", "vs AI", "Tournament"].includes(matchType)) {
        return reply.status(400).send({ error: "Invalid match type" });
      }
  ...
  // // Submit match results and update user stats accordingly
